...
logs backing every number reported in the current version of the paper (the
version submitted to Neurocomputing). File names are kept exactly as produced
by the training/evaluation pipeline for provenance; timestamps in logs/ are the
actual run times.
...
- **COGS**: eval_rel_copy_s{42,43,44}_dev.json / _gen.json, eval_std_copy_s{42,43,44}_dev.json / _gen.json -- reproduces the COGS dev/gen numbers in Table 2 and Sec. "COGS: Generalization Split".
